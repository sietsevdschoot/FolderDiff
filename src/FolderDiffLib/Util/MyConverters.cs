...
namespace FolderDiffLib.Util
{
    public static class MyConverters
    {
        ...
        {
            if (!typeof(TSrcEnum).IsEnum)
                throw new ArgumentException("Expected enum type", "TSrcEnum");
            if (!typeof(TDestEnum).IsEnum)
                throw new ArgumentException("Expected enum type", "TDestEnum");
